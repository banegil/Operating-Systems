...
void usage(int argc, char *argv[]){
	...
	fprintf(stderr,"\t-n <cpus>: Sets number of CPUs for the simulator\n");
	...
	fprintf(stderr,"\t-q <quantum>: Set up the timeslice or quantum for the RR algorithm\n");
	fprintf(stderr,"\t-l <period>: Set up the load balancing period (specified in simulation steps)\n");
...
int main(int argc, char *argv[]) {
	...
	while ((optc = getopt(argc, argv, "+hi:n:s:dt:m:q:pl:L")) != (char)-1) {
		...
		case 's':
			...
			while(strcmp(available_schedulers[i].sched_name,optarg)!=0) {i++;}
